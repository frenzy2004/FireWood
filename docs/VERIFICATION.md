...
A broader live-orchard prompt also selected `inspect_asset` and received current live evidence. Synthesis failed the conservative claim-level validator because the model described evidence in synonyms the payload did not contain; evidence tools now emit a plain-language summary. Measured across 23 fixture runs, 5 produced a grounded briefing, with the rate swinging between 3 of 3 and 0 of 8 across sampling sessions. Treat a grounded paragraph as a bonus. The tool trace is the reliable evidence surface and is exact on every run. Unsupported prose is still replaced by the safe fallback, and the deterministic panels and visible tool result remain authoritative.
