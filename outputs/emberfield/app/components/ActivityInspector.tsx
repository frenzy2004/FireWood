...
export function ActivityInspector({
  ...
  return <aside className="inspector panel" aria-label="Activity inspector">
    ...
    {replayCutoff ? <p className="replay-note"><strong>Replay cutoff {formatUtc(replayCutoff)}</strong><br />Raw detections and observations are filtered to this time. The context score remains the latest completed deterministic assessment.</p> : null}
    ...
    <section className="source-freshness"><p className="eyebrow">Source freshness</p>{Object.values(snapshot.sources).map((source) => <div key={source.source} className={`source-row source-${source.status}`}><span><strong>{source.source}</strong><small>{source.mode} · {title(source.status)}</small></span><small>fetched {formatUtc(source.fetchedAt)}<br />observed {formatUtc(source.observedAt)}{source.error ? <><br />{source.error.code}: {source.error.message}</> : null}</small></div>)}</section>
